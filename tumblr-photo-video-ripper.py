...
class DownloadWorker(Thread):
    ...
    def download(self, medium_type, post, target_folder):
        try:
            ...
            if medium_url is not None:
                self._download(medium_type, medium_url, target_folder,num)
        except TypeError:
            pass
    ...
    def _download(self, medium_type, medium_url, target_folder,num):
        ...
        if not os.path.isfile(file_path):
            ...
            while retry_times < RETRY:
                try:
                    ...
                    json_body = [
                        {
                            "measurement": "media_num",
                            "tags": {
                                "media_num_total": "int_num"
                            },
                            "fields": {
                                "downloaded_media_total_num": 1
                            }
                        }
                    ]
                    client = InfluxDBClient('localhost', 8086, 'root', '', 'tumblr_crawler_downmedia_num_db')
